...
class TestTicketService(unittest.TestCase):
    def setUp(self) -> None:
        self.repl = Repl(repl_displayer=TestDisplayer(), is_show_prompt=False, show_welcome=False)
        return super().setUp()

    def test_repl_help(self):
        self.repl.add_command("t1", self.dummy_action, "t1 help string")

        with patch('builtins.input', return_value="help"), patch('sys.stdout', new=StringIO()) as o:
    ...
    def test_show_prompt(self):
        ...
        with_prompt_repl = Repl(repl_displayer=TestDisplayer(), is_show_prompt=True, show_welcome=False)
        with patch('sys.stdout', new=StringIO()) as o:
    ...
    def test_execute_action(self):
        ...
        with patch('builtins.input', return_value="hello"), patch('sys.stdout', new=StringIO()) as o:
            self.repl.run(single_command=True)
            self.assertEqual(o.getvalue(), "hello world\n")
    
    def test_quit_action(self):
        self.repl.add_command("quit", quit, "Exit the program")
        with patch('builtins.input', return_value="quit"), patch('sys.stdout', new=StringIO()) as o:
